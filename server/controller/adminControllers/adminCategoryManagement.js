...
exports.adminategoryDelete= async (req, res) => {
  console.log(101)
    ...
    res.redirect("/admin_category_view_get")
}
...
exports.adminImageDeletePost= async (req, res) => {
  ...
  try {
    const category = await Category.findOne({ "imageUrl._id": id });
    if (category) {
      const imageIndex = category.imageUrl.findIndex(
        (image) => image._id.toString() === id
      );
      if (imageIndex > -1) {
        category.imageUrl.splice(imageIndex, 1);
        await category.save();
        return res.status(200).end();
      }
    }
