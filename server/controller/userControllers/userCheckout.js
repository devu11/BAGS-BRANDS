...
// Add a new route to fetch available coupons
exports.availableCoupon= async (req, res) => {
  try {
    const availableCoupons = await Coupons.find(); // Assuming you have a 'Coupons' model
...
exports.userCheckOutPost = async (req, res) => {
  try {
    ...
    async function placeOrder() {
      ...
      await order.save().then((res) => {

        req.session.orderId = res.orderId
...
exports.userCouponCheck = async (req, res) => {
  ...
  totalAmountInCart += price * product.quantity;
      ...
      console.log("coupon min dis",couponMinDiscount)
      const couponMaxDiscount = coupon.maxDiscount;
      console.log("coupon max dis",couponMaxDiscount)

      let discountAmount = totalAmountInCart * (couponDiscount / 100);
      console.log("dis amnt",discountAmount)
      if (discountAmount > coupon.maxDiscount) {
        discountAmount = coupon.maxDiscount;
        console.log(9898,discountAmount);
      }
